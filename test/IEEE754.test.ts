...
/// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
import { IS_LITTLE_ENDIAN } from "@/platform/system";
...
test("float32 read & write", () => {
    ...
    const readResult: number = read(buffer, 0, IS_LITTLE_ENDIAN, 23, 4);
    const writeResult: Uint8Array = new Uint8Array(4);

    write(writeResult, input, 0, IS_LITTLE_ENDIAN, 23, 4);
...
test("float64 read & write", () => {
    ...
    const readResult: number = read(buffer, 0, IS_LITTLE_ENDIAN, 52, 8);
    const writeResult: Uint8Array = new Uint8Array(8);

    write(writeResult, input, 0, IS_LITTLE_ENDIAN, 52, 8);
