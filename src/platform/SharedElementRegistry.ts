...
class SharedElementRegistry {
    ...
    public static create(tagName: string): HTMLElement {
        if (!SharedElementRegistry.contains(tagName)) {
            SharedElementRegistry.register(tagName, document.createElement(tagName));
        }
        return SharedElementRegistry.__ELEMENT_MAP__[tagName];
    }
    ...
    public static register(tagName: string, element: HTMLElement): void {
        SharedElementRegistry.__ELEMENT_MAP__[tagName] = element;
    }
}
